...
const count = (target, input) => {
  const total = input.reduce((accumulator, value) => {
    const filtered = value.filter(val => val === target);
    accumulator += filtered.length;
    return accumulator
  }, 0);
  return total;
};
...
const totalSum = (input) => {
  const allSum = input.reduce((accumulator, value) => {
...
const divisibleByFiveTwoToThePower = (input) => {
  const findNumbers = input.map(value => {
    const divisibleByFive = value.reduce((accumulator, number) => {
      if (number % 5 === 0 && typeof number !== 'string') {
        accumulator.push(Math.pow(2, number));
      }
      return accumulator;
    }, []);
...
let findMaleAndFemale = (data) => {
  const hasGender = data.reduce((accumulator, value) => {
...
let findShortest = (data) => {
  ...
  const shortest = data.reduce((accumulator, value) => {
    if (value.height < shortestHeight) {
      shortestHeight = parseInt(value.height);
      accumulator = value.name;
    }
    console.log('shortheight', shortestHeight);
    console.log('shortestname', accumulator);
    return accumulator;
  }, '');
  return shortest;
}
