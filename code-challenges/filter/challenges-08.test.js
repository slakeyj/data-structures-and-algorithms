...
const filterStringsWithVowels = (arr) => {
  ...
  const vowelWords = arr.filter(value => value.match(regEx));
  return vowelWords
};
...
const notInFirstArray = (forbiddenValues, arr) =>
  arr.filter(value => forbiddenValues.indexOf(value) === -1);
...
const getBaseStatGreaterThan = (arr, minBaseStat) =>
  arr.filter(stat => stat.baseStat > minBaseStat);
...
const getStatName = (arr, minBaseStat) => {
  const aboveMin = arr.filter(stat => stat.baseStat > minBaseStat);
  return aboveMin.map(thingy => thingy.stat.name);
};
